...
class JsonSocket(object):
    def __init__(self, address, port, encrypt = False):
        ...
        if self._ssl != True:
            ...
            self.socket = context.wrap_socket(sock, server_hostname=DEFAULT_XAPI_ADDRESS )
...
def loginCommand(userId, password, appName=''):
    return baseCommand('login', dict(userId=os.getenv('XTB_USERNAME'), password=os.getenv('XTB_PASSWORD'), appName=appName))
